puertos = new Array("3306", "5432", "1521");

function puertoPredeterminado(formulario, control, nombre) {
	var seleccion = document.getElementById("dbsys");
	var puerto= document.getElementById("dbPuerto");
	puerto.value=puertos[seleccion.selectedIndex];

}

function abrir_ventana(url, nombre) {

	var ventana = window.open(url, nombre,
			"resizable,status,menubar,width=850,height=400,scrollbars");
	return ventana;
}
...
function comparar(formulario) {
	...
	while (formulario.elements[control]) {
		...
		if ((formulario.elements[control].name.substring(0, cadena.length) != 'observacion_')
				&& (formulario.elements[control].name != 'id_esquema')) {
			if (!isNaN(formulario.elements[control].value)) {

				mi_matriz[indice] = parseInt(formulario.elements[control].value);
				// alert
				// (formulario.elements[control].name+'->'+mi_matriz[indice]);
				indice++;
	...
	for (a = 0; a < (mi_matriz.length - 1); a++) {
		...
		if (resultado == 1) {
			alert1("Se esperaba no tener valores duplicados:" + mi_matriz[a]
					+ '-' + mi_matriz[(a + 1)]);
			return false;
		}
...
function acentos(x) {
	...
	// Also accepts and converts single and double quotation marks, square and
	// angle brackets
...
/**
 * Asegura que los valores enviados en un formulario es numérico y está en un
 * rango determinado
 * 
 * @param object
 *            formulario
...
/**
 * Muestra un mensaje de error si el tamaño de la cadena es inferior a un número
 * dado
 * 
 * 
 * @param object
...
function longitud_cadena(formulario, control, tamanno) {
	...
	if (longitud < tamanno) {
		campo.select();
		alert1('Dato incorrecto. Por seguridad el campo debe contener un dato con más de '
				+ tamanno + ' caracteres');
		campo.focus();
		return false;
	}
...
/*******************************************************************************
 * Muestra un mensaje de error si no se ha seleccionado una opción y se pretende
 * salir
 * 
 * 
 * @param object
...
function seleccion_valida(formulario, control) {
	/*
	 * var campo= formulario.elements[control]; var valor= campo.value;
	 * alert1('Debe seleccionar una opción.');
	 * 
	 * 
	 * if (valor!=-1) { campo.select(); alert1('Debe seleccionar una opción.');
