...
$(document).ready(function() {
	...
	function ordenTrabajo(){
		...
		$.ajax({
			...
			dataType: "json",
			data: { metodo:'paquetesTrabajo', id_proyecto: 15},
			success: function(data){
				
				<!-- data = unique(data); -->
				
				$.each(data , function(indice,valor){
					if(data[ indice ].cf_44 == "Comisionamiento"){
						$("<option value='"+data[ indice ].id +"'>"+data[ indice ].subject + "</option>").appendTo("#<?php echo $this->campoSeguro('ordenTrabajo');?>");
					}
