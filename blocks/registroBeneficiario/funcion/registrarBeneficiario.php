...
class Registrar
{
    ...
    public function validarGeolocalizacion()
    {
        ...
        if (isset($_REQUEST['geolocalizacion']) && $_REQUEST['geolocalizacion'] != null && $_REQUEST['geolocalizacion'] != '') {
            $arreglo = explode(",", $_REQUEST['geolocalizacion']);

            $latitud = trim($arreglo[0]);
            $longitud = trim($arreglo[1]);

            if (!is_numeric($longitud)) {

                redireccion::redireccionar('errorGeolocalizacionNoValida');

            } elseif ($longitud < -77 || $longitud > -73) {

                redireccion::redireccionar('errorRangoLongitud');
            ...
            } elseif ($latitud > 10 || $latitud < 6) {

                redireccion::redireccionar('errorRangoLatitud');

            }
        } else {

            redireccion::redireccionar('errorGeolocalizacion');
    ...
    public function procesarFormulario()
    {
        ...
        /**
         * Validar Geolocalización que es Obligatortia
         **/

        $this->validarGeolocalizacion();
