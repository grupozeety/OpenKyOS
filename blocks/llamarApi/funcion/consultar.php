...
class Consultar {
	...
	/**
	 * SINCRONIZACIÓN COMISIONAMIENTO OPENKYOS - ERPNEXT
	 * Funciones Sincronización Comisionamiento Aplicativos
	 * Autor: Sosa,V
	 * Version : 1.0.0.0
	 * Fecha : 2016/10/22
	 */
	...
	/**
	 * Funcion Consultar Ordenes de Trabajo en General el ERPNext
	 * Autor: Sosa, V.
	 * Version : 1.0.0.0
	 * Fecha : 2016/10/24
	 */
	...
	/**
	 * Funcion Consultar la lista de Materiales asociados al comisionamiento
	 * Autor: Sosa, V.
	 * Version : 1.0.0.0
	 * Fecha : 2016/10/24
	 * Debe ser un BOM con estado kit_comisionamiento=1
	 */
	public function consultarKit($datosConexion = '') {
		$this->configurarERPNext ( $datosConexion );
		$data = array (
				"kit_comision" => 1 
		);
	...
	/**
	 * Funcion Crear Carpetas en Alfresco
	 * Autor: Sosa, V.
	 * Version : 1.0.0.0
	 * Fecha : 2016/10/13
	 */
	public function crearCarpeta($datosConexion, $variable = '') {
		// Esta funcion es un ejemplo para referencias futuras
		$url = "http://" . $datosConexion ['host'] . "/alfresco/service/api/site/folder/" . $variable [$key] ['site'] . "/documentLibrary/" . $variable [$key] ['padre'] . "/" . $variable [$key] ['hijo']; // pendiente la pagina para modificar parametro
		$carpeta = array (
				'name' => 'nombreCarpeta',
				'type' => 'cm:folder' 
		);
		
		$result = RestClient::post ( $url, json_encode ( $carpeta ), 'usuario', 'constraseña' ); // removed $input as suggested by Dan J
		$json_decode = json_decode ( $result->getResponse (), true );
	}
}
?>
