...
class FrappeClient {
	...
	private $_curl_timeout = 60;
	private $_limit_page_length =100;
	
	
	
	function configurar($datos) {
		
		$this->_cookie_file=$datos ['cookie_file'];
		if (! is_writable ( dirname ( $this->_cookie_file ) )) {
			throw new FrappeClient_Exception ( "Cookie file not writable:" . $this->_cookie_file, 2 );
		}
		
		
		$this->_auth_url = $datos['host'].$datos['auth_url'];
	...
	private function _auth_check() {
		if ($this->is_auth == false) {
			throw new FrappeClient_Exception ( "Auth check fail.", 3 );
		} else {
			return true;
		}
	}
	private function _auth() {
		...
		if (isset ( $login_result->body->message )) {
			...
			throw new FrappeClient_Exception ( "Auth fail.", 4 );
		}
		if (! is_file ( $this->_cookie_file )) {
			throw new FrappeClient_Exception ( "Cookie file not found:" . $this->_cookie_file, 5 );
		}
	}
	public function get($doctype, $key) {
		$this->_auth_check ();
		return $this->_curl ( 'GET', array (
				'doctype' => $doctype,
				'data' => $key 
		) );
	}
	public function search($doctype, $conditions = array(), $fields = array(), $limit_start = 0, $limit_page_length = 0) {
		$this->_auth_check ();
		...
		return $this->_curl ( 'SEARCH', array (
				'doctype' => $doctype,
				'filters' => $conditions,
				'fields' => $fields,
	...
	public function update($doctype, $key, $params) {
		return $this->_curl ( 'UPDATE', array (
				'doctype' => $doctype,
				'key' => $key,
				'data' => $params 
		) );
	...
	public function docstatus($doctype, $params) {
		$this->_auth_check ();
		return $this->_curl ( 'DOCSTATUS', array (
				'doctype' => $doctype,
				'data' => $params 
		) );
	}
	public function delete($doctype, $key) {
		$this->_auth_check ();
		return $this->_curl ( 'DELETE', array (
				'doctype' => $doctype,
				'key' => $key 
		) );
	}
	private function _curl($method = 'GET', $params = array()) {
		switch ($method) {
			case 'AUTH' :
				$url = $this->_auth_url;				
				$ch = curl_init ( $url );
		...
		if (! empty ( $this->_basic_auth )) {
			...
			curl_setopt ( $ch, CURLOPT_USERPWD, $this->_basic_auth ['usr'] . ':' . $this->_basic_auth ['pwd'] );
		}
