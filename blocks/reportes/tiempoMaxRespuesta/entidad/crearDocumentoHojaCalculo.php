...
namespace reportes\tiempoMaxRespuesta\entidad;
...
public function __construct($sql, $informacion,$informacion1,$informacion2) {
        ...
        if($this->informacion)
        {
          foreach ($this->informacion as $key => $value) {
            ...
            //  exit;
              ...
              $this->objCal->setActiveSheetIndex(0)->setCellValue('C' . $i, $value['t2'])->getStyle('C' . $i)->applyFromArray($styleCentradoVertical);

              $this->objCal->setActiveSheetIndex(0)->setCellValue('D' . $i, $value['t3'])->getStyle('D' . $i)->applyFromArray($styleCentradoVertical);

              $this->objCal->setActiveSheetIndex(0)->setCellValue('E' . $i, $value['tiempo_respuesta'])->getStyle('E' . $i)->applyFromArray($styleCentradoVertical);

              $this->objCal->setActiveSheetIndex(0)->setCellValue('F' . $i, $value['condicion_obtenida'])->getStyle('F' . $i)->applyFromArray($styleCentradoVertical);
          ...
          if($this->informacion1)
          {
              foreach ($this->informacion1 as $key => $value) {
                ...
                //  exit;
                  ...
                  $this->objCal2->setCellValue('C' . $i, $value['person_llamadas'])->getStyle('C' . $i)->applyFromArray($styleCentradoVertical);

                  $this->objCal2->setCellValue('D' . $i, $value['dentro_llamadas'])->getStyle('D' . $i)->applyFromArray($styleCentradoVertical);

                  $this->objCal2->setCellValue('E' . $i, $value['fuera_llamadas'])->getStyle('E' . $i)->applyFromArray($styleCentradoVertical);

                  $this->objCal2->setCellValue('F' . $i, $value['calculo_indicador'])->getStyle('F' . $i)->applyFromArray($styleCentradoVertical);
          ...
          if($this->informacion2)
          {
            foreach ($this->informacion2 as $key => $value) {
              ...
              //  exit;
                ...
                $this->objCal3->setCellValue('C' . $i, $value['person_llamadas'])->getStyle('C' . $i)->applyFromArray($styleCentradoVertical);

                $this->objCal3->setCellValue('D' . $i, $value['dentro_llamadas'])->getStyle('D' . $i)->applyFromArray($styleCentradoVertical);

                $this->objCal3->setCellValue('E' . $i, $value['fuera_llamadas'])->getStyle('E' . $i)->applyFromArray($styleCentradoVertical);

                $this->objCal3->setCellValue('F' . $i, $value['calculo_indicador'])->getStyle('F' . $i)->applyFromArray($styleCentradoVertical);
    ...
    public function generarEsquemaDocumento() {
      ...
      //Hoja 1
        ...
        $this->objCal->setActiveSheetIndex(0)->setCellValue('B1', 'T1 - Fecha recepción')->getStyle("B1")->applyFromArray($styleCentrado);

        $this->objCal->setActiveSheetIndex(0)->setCellValue('C1', 'T2 - Agente responde')->getStyle("C1")->applyFromArray($styleCentrado);

        $this->objCal->setActiveSheetIndex(0)->setCellValue('D1', 'T3 - Atención personalizada')->getStyle("D1")->applyFromArray($styleCentrado);

        $this->objCal->setActiveSheetIndex(0)->setCellValue('E1', 'Tiempo de respuesta (T3 - T2)')->getStyle("E1")->applyFromArray($styleCentrado);

        $this->objCal->setActiveSheetIndex(0)->setCellValue('F1', 'Condición obtenida')->getStyle("F1")->applyFromArray($styleCentrado);
      ...
      //Hoja 2
            {
              ...
              $this->objCal2->setCellValue('D1', 'Llamadas dentro del tiempo de respuesta')->getStyle("D1")->applyFromArray($styleCentrado);
            ...
            //Hoja 3
                  {
                    ...
                    $this->objCal3->setCellValue('D1', 'Llamadas dentro del tiempo de respuesta')->getStyle("D1")->applyFromArray($styleCentrado);

                    $this->objCal3->setCellValue('E1', 'Llamadas que excedieron el tiempo máximo de respuesta')->getStyle("E1")->applyFromArray($styleCentrado);

                    $this->objCal3->setCellValue('F1', 'Indicador tiempo máximo de respuesta')->getStyle("F1")->applyFromArray($styleCentrado);
    ...
    public function configurarDocumento() {
        ...
        $this->objCal->getActiveSheet()->setTitle('tiempoMaxRespuesta');

        $this->objCal2 = $this->objCal->createSheet();
        $this->objCal2->setTitle('IndicadortiempoMaxRespuesta');
    ...
    public function retornarDocumento() {
        ...
        //      exit;
            ...
            header('Content-Disposition: attachment;filename="ReportestiempoMaxRespuesta' . time() . '.xlsx"');
