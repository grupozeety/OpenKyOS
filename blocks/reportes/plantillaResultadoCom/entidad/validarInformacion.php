		...
		foreach ( $this->datos_beneficiario as $key => $value ) {
			...
			if ($value ['fecha_contrato']) {
				...
				if (! preg_match ( $date_regex, $hiredate )) {
					...
					$mensaje = " La fecha de contrato asociado al beneficiario con identificación " . $value ['identificacion_beneficiario'] . ", no es valida.Sugerencia verifique que la columna Fecha de Contrato del Plantilla  se en formato texto y con esl formato 'yyyy-mm-dd'.";
