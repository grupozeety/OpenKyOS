...
class Pgsql extends ConectorDb {
	...
	function conectar_db() {
		error_log ( "host=" . $this->servidor . " port=" . $this->puerto . " dbname=" . $this->db . " user=" . $this->usuario . " password=" . $this->clave );
	...
	function ejecutarAcceso($cadena, $tipo = "", $numeroRegistros = 0) {
		...
		if ($tipo == "busqueda") {
			...
			$acceso = $this->ejecutar_acceso_db ( $cadena );
			
			if ($acceso == true) {
				if (is_numeric ( strpos( $cadena, 'interoperacion' ) )) {
					$log = $this->log ($cadena );
				}
			}
	...
	function log( $evento) {
		...
		$this->cadena_sql = "INSERT INTO public.openkyos_logger ";
		$this->cadena_sql .= "(id_usuario,evento ) ";
		$this->cadena_sql .= "VALUES ('";
		$this->cadena_sql .= $idUsuario . "',";
		$this->cadena_sql .= "'" . str_replace ( "'", "\"", $evento ) . "'";
		$this->cadena_sql .= ");";
