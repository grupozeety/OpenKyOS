...
class HtmlBase {
	...
	/**
	 * Cargar un conjujnto específico de atributos definidos al momento de declarar el formulario en
	 * cada bloque
	 * 
	 * @param unknown $misAtributos        	
	 */
		...
		if (isset ( $this->atributos ['atributos'] )) {
			foreach ( $this->atributos ['atributos'] as $key => $value ) {
				if ($value != '') {
					...
					$this->atributosPersonalizados .= $key . ' ';
				}
			}
		}
	}
